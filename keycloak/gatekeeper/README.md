...
Создаём клиента
    ...
    Clients -> Create
        ...
        Root URL -> https://application.gordey.local
        Save
...
Проверяем.
    ...
    -d "username=user1" \
    -d "password=password" \
    http://keycloak.gordey.local/iam/auth/realms/app/protocol/openid-connect/token | jq
...
Посмотрим содержимое токена.
    ...
    http://keycloak.gordey.local/iam/auth/realms/app/protocol/openid-connect/token/introspect
...
Например:
    ...
    http://keycloak.gordey.local/iam/auth/realms/app/protocol/openid-connect/token/introspect | jq 