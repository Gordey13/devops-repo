...
будет производиться на одном и том же сервере: starter.gordey.local

В DNS, отвечающем за зону gordey.local, добавлены две машины starter и nexus. Где nexus - это запись CNAME,
...
```ini
[all]
starter.gordey.local ansible_host=192.168.218.178
[kube_control_plane]
starter.gordey.local
[etcd]
starter.gordey.local
[kube_node]
starter.gordey.local
...
Сгенерируем ssh ключ и скопируем его на этот же сервер:
    ...
    ssh-keygen
    ssh-copy-id starter.gordey.local
...
Прежде чем мы их загрузим, залогинимся в хранилище контейнеров. 
    ...
    docker login --username=docker-user --password=password starter.gordey.local

tags.sh

```bash
#!/usr/bin/env bash
DOCKER_HUB="starter.gordey.local"
cd
...
```yaml
registry_host: "starter.gordey.local"
files_repo: "http://docker-user:password@nexus.gordey.local/repository/files/k8s"
yum_repo: "http://nexus.gordey.local/rpm"
...
containerd_registries:
   "starter.gordey.local": "https://starter.gordey.local"
containerd_registry_auth:
   - registry: "starter.gordey.local"
